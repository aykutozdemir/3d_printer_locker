...
void EventHandlerTask::on_start()
{
    ...
    logInfo(F("Task started, listening for button and MB light sensor events"));
}
...
void EventHandlerTask::on_msg(const MsgData &msg)
{
    switch (msg.type)
    {
        case EVT_BUTTON_SHORT_CLICK:
            if (isDimming)
            {
                logInfo(F("EVENT: Short click received - stopping dimming and saving level"));
                publish(TOPIC_LIGHT_EVENTS, EVT_LIGHT_DIM_STOP, 0, nullptr);
                isDimming = false;
            }
            else
            {
                logInfo(F("EVENT: Short click received - toggling light"));
        ...
        case EVT_BUTTON_LONG_CLICK:
            logInfo(F("EVENT: Long click received - starting dimming mode"));
        ...
        case EVT_MB_LIGHT_SENSOR_CHANGED:
            ...
            if (msg.arg == 1)
            {
                logInfo(F("EVENT: MB light sensor HIGH - turning light ON"));
                publish(TOPIC_LIGHT_EVENTS, EVT_LIGHT_TOGGLE, 1, nullptr); // Force ON
            }
            else
            {
                logInfo(F("EVENT: MB light sensor LOW - turning light OFF"));
