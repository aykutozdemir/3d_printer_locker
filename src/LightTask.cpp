...
LightTask::LightTask() : Task(F("Light"))
{
    setPeriod(50); // Update every 50ms for smooth dimming
...
void LightTask::on_start()
{
    ...
    if (state == 1)
    {
        ...
        logInfo(F("Light restored ON from EEPROM"));
    }
    else
    {
        setLightState(false);
...
void LightTask::step()
{
    ...
    if (currentState == LIGHT_DIMMING)
    {
        if (dimStepTimer.isExpired())
        {
            dimStepTimer.startTimer(DIM_STEP_INTERVAL_MS);
            if (dimIncreasing)
            {
            ...
            logDebugf(F("Dim level: %u%%"), currentDimLevel);
        }
    }
...
void LightTask::saveDimLevel()
{
    ...
    logInfof(F("Dim level saved to EEPROM: %u%%"), savedDimLevel);
}
