...
KeypadTask::KeypadTask() : Task(F("Keypad"))
{
    setPeriod(10); // Check keypad every 10ms for responsive debouncing
}
...
void KeypadTask::on_start()
{
    ...
    for (uint8_t i = 0; i < 4; i++)
    {
        ...
        // Timer will be initialized when key is pressed
        keys[i].debounced = false;
        keys[i].longReported = false;
    }

    logInfo(F("Started - 4-digit keypad"));
}
...
void KeypadTask::checkKey(uint8_t keyIndex, uint8_t pin, uint8_t eventType)
{
    ...
    {
        // Key just pressed
        keys[keyIndex].debounceTimer = createTimerTyped<Timer8>(DEBOUNCE_TIME_MS);
        keys[keyIndex].longPressTimer = createTimerTyped<Timer16>(LONG_PRESS_TIME_MS);
        keys[keyIndex].debounced = false;
        keys[keyIndex].longReported = false;
        logDebugf(F("Key %u press detected"), keyIndex + 1);
    }
    // Detect key release
    else if (keys[keyIndex].currentState == HIGH && keys[keyIndex].lastState == LOW)
    {
        // Key just released
        if (!keys[keyIndex].debounced && keys[keyIndex].debounceTimer.isExpired() && !keys[keyIndex].longReported)
        {
...
{
    logInfof(F("Key %u pressed"), keyIndex + 1);

    // Publish key press event
    publish(TOPIC_KEYPAD_EVENTS, eventType, 0);
...
{
    logInfof(F("Key %u long"), keyIndex + 1);
    publish(TOPIC_KEYPAD_EVENTS, eventType, 0);
}
